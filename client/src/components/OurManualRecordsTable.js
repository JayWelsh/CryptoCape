...
const rows = [
  ...
  { id: 'type', numeric: true, disablePadding: false, label: 'Buy / Sell' },
];
...
class EnhancedTable extends React.Component {
  ...
  componentDidUpdate(prevProps) {
      if(JSON.stringify(this.props.timeseries) !== JSON.stringify(this.state.data)) {
          this.setState({
              data: this.props.timeseries
          })
      }
      if(this.props.symbol !== this.state.symbol || this.props.tokenId !== this.state.tokenId) {
          this.setState({
            symbol: this.props.symbol,
            tokenId: this.props.tokenId
          })
      }
      if(this.props.publicKey !== this.state.publicKey) {
        this.setState({
  ...
  render() {
    const { classes, refetchData, handleClose } = this.props;
    const { data, order, orderBy, selected, rowsPerPage, page, symbol, tokenId, publicKey } = this.state;
    const emptyRows = rowsPerPage - Math.min(rowsPerPage, data.length - page * rowsPerPage);

    return (
      <div className={classes.root}>
        <EnhancedTableToolbar handleClose={handleClose} refetchData={refetchData} publicKey={publicKey} tokenId={tokenId} numSelected={selected.length} selected={selected} />
                ...
                .map(n => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="right">{priceFormat(n.price, 2, symbol)}</TableCell>
                      <TableCell align="right">{n.type === 'buy' ? "Buy" : "Sell"}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
